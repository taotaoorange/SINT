...
void NormalizeLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i=0; i<n; ++i) {
    //LOG(INFO) << bottom_data[i*d] << "  " << *(bottom_data+i*d);
    Dtype normsqr = caffe_cpu_asum<Dtype>(d, squared_data+i*d);
    normsqr = normsqr > Dtype(0) ? normsqr : Dtype(1);
...
void NormalizeLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  int d = top[0]->count() / n;
  for (int i=0; i<n; ++i) {
    Dtype normsqr = caffe_cpu_dot(d, bottom_data+i*d, bottom_data+i*d);
    if (normsqr > Dtype(0)) {
      ...
      caffe_cpu_scale(d, Dtype(pow(normsqr, -0.5)), bottom_diff+i*d, bottom_diff+i*d);
    } else {
      caffe_set(d, Dtype(0), bottom_diff+i*d);
    }
  }
}
