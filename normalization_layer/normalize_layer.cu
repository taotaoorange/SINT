...
void NormalizeLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  Dtype normsqr;
  for (int i=0; i<n; ++i) {
    caffe_gpu_dot(d, bottom_data+i*d, bottom_data+i*d, &normsqr);
    if (normsqr > Dtype(0)) {
      ...
      caffe_gpu_scale(d, Dtype(pow(normsqr, -0.5)), bottom_diff+i*d, bottom_diff+i*d);
    } else {
      caffe_gpu_set(d, Dtype(0), bottom_diff+i*d);
    }
  }
}
